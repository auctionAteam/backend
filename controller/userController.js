  ...
  try {
    ...
    if (!loginUser[0]) {
      return res
        ...
        .json({ message: "없는 아이디입니다." });
    }

    const hashPassword = crypto
      .pbkdf2Sync(password, loginUser[0].salt, 10000, 10, "sha512")
      .toString("base64");
    if (loginUser[0].password !== hashPassword) {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .json({ message: "아이디 또는 비밀번호를 다시 확인해주세요." });
    }
  ...
  try {
    ...
    if (state) {
      items = await Promise.all(
        itemIds.map((itemId) =>
          itemService.findFilterItemInfoByItemId(
            itemId,
            state,
            limit,
            currentPage
